...
class OrchestratorAgent(Talk2DrawingsBaseAgent):
    ...
    def normalize_value(self, value):
        if pd.isna(value) or value in ['Not Found', 'N/A', 'na', 'none']:
            return value
    ...
    async def process(self, input_data: Dict[str, Any], context: Dict[str, Any] = None) -> Dict[str, Any]:
        try:
            ...
            final_results = await self._generate_final_results_with_coordinates(ocr_result, qa_result, pdf_path, config)
    ...
    async def _generate_final_results(self, ocr_result: Dict, qa_result: Dict, pdf_path: str, config: Dict) -> Dict[str, Any]:
        ...
        json_results = {'document_info': ocr_result['document_info'],
            'processing_summary': {
                ...
                'timestamp': timestamp}, 'results': results_data}
    ...
    async def _generate_final_results_merged(self, merged_data: Dict, qa_result: Dict, output_dir: str, config: Dict) -> Dict[str, Any]:
        ...
        df = pd.DataFrame(results_data)
        df['Normalized_Answer'] = df['Answer'].apply(self.normalize_value)
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        ...
        json_results = {'processing_mode': 'merged_json',
            ...
            'processing_summary': {
                ...
                'defaults_applied': len([r for r in results_data if r['Deflection_Default'] == 'Applied']),
                'timestamp': timestamp,
                'source_files': source_pdfs}, 'results': results_data}
    ...
    async def _generate_final_results_with_coordinates(self, ocr_result: Dict, qa_result: Dict, pdf_path: str,
                                                       config: Dict) -> Dict[str, Any]:
        try:
            questions = self.qa_agent.questions
            raw_answers = qa_result['qa_results']

            # Apply deflection defaults with defensive handling
            ...
            # Normalize with defensive handling
        ...
        # Final safety check with debug
        print(f"DEBUG: processed_answers type = {type(processed_answers)}")
        if not isinstance(processed_answers, dict):
            print(f"CRITICAL: processed_answers is {type(processed_answers)}, content: {processed_answers}")
            processed_answers = raw_answers if isinstance(raw_answers, dict) else {}

        print(f"DEBUG: About to process {len(processed_answers)} answers")

        results_data = []
        for i, question in enumerate(questions, 1):
            qid = f"Q{i}"
            answer_data = processed_answers.get(qid, {})
            print(f"DEBUG Q{i}: type={type(answer_data)}, content={str(answer_data)[:100]}")
            ...
            result_row = {'Question_Number': i,
                          ...
                          'Coordinate_Width': coordinate_summary.get('width', 0) if isinstance(coordinate_summary,
                                                                                               dict) else 0,
                          'Coordinate_Height': coordinate_summary.get('height', 0) if isinstance(coordinate_summary,
                                                                                                 dict) else 0,
                          'Match_Type': coordinate_summary.get('match_type', 'none') if isinstance(coordinate_summary,
                                                                                                   dict) else 'none',
                          'Match_Confidence': f"{coordinate_summary.get('match_confidence', 0):.1f}%" if isinstance(
                              coordinate_summary, dict) else "0%",
                          'Coordinates_Text': format_coordinates_for_output(
                              coordinates) if coordinates else "No coordinates",
        ...
        project_name = Path(pdf_path).stem
        output_dir = Path(pdf_path).parent
        export_paths = export_results_with_coordinates(results_data, str(output_dir), "pipeline_results_coords",
                                                       project_name)
        ...
        json_results = {'document_info': ocr_result['document_info'],
                        'coordinate_tracking': {'enabled': True,
                                                'algorithm': 'fuzzy_text_matching',
                                                'success_rate': qa_result.get('coordinates_found', 0) / len(
                                                    questions) * 100,
        ...
        return {'csv_path': str(standard_csv_path),
                'json_path': str(json_path),
