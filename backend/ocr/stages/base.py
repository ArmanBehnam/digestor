...
class PDFTextStage(BaseProcessingStage):
    ...
    def process(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._log_stage_complete(context,  pages=result.total_pages,
                               text_length=len(result.extracted_text), tables=len(pdf_data.get('tables', [])))
        return context
...
class OCRStage(BaseProcessingStage):
    ...
    def process(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result.images = [{'page_number': page_num, 'format': 'array', 'dimensions': img.shape[:2]}
