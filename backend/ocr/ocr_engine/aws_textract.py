...
class AWSTextractEngine(BaseOCREngine):
    ...
    def _initialize_client(self) -> None:
        try:
            ...
            boto_config = Config( read_timeout=900, connect_timeout=900,
            ...
            if access_key and secret_key:
                ...
                self._textract_client = boto3.client('textract', region_name=region, config=boto_config)
    ...
    def _extract_text_impl(self, image: np.ndarray, page_num: int, **kwargs) -> List[ExtractedElement]:
        ...
        if len(image_bytes) > 10 * 1024 * 1024:
            raise OCRExtractionError(self.name, page_num,
                f"Image too large: {len(image_bytes) / 1024 / 1024:.1f}MB (max: 10MB)")

        try:
            response = self._textract_client.detect_document_text(Document={'Bytes': image_bytes})
    ...
    def _process_textract_response(self, response: Dict[str, Any],
        ...
        for block in response.get('Blocks', []):
            if block['BlockType'] == 'WORD':
                ...
                bbox = {'x': int(bbox_data['Left'] * width),
                ...
                if 'Polygon' in block['Geometry']:
                    ...
                    for point in block['Geometry']['Polygon']:
                        polygon.append({'x': point['X'] * width, 'y': point['Y'] * height})
    ...
    def _extract_table_data(self, table_block: Dict[str, Any], all_blocks: Dict[str, Any],
        ...
        try:
            ...
            table_bbox = {'x': int(bbox_data['Left'] * width),
                'y': int(bbox_data['Top'] * height),
                'width': int(bbox_data['Width'] * width),
                'height': int(bbox_data['Height'] * height)}
            ...
            return {'page_number': page_num, 'bbox': table_bbox, 'rows': table_rows,
                'headers': table_rows[0] if table_rows else [],
                'confidence': table_block.get('Confidence', 0) / 100.0,
                'table_id': table_block['Id'], 'cell_count': len(cells)}
        except Exception as e:
    ...
    def _extract_cell_data(self, cell_block: Dict[str, Any], all_blocks: Dict[str, Any],
        ...
        try:
            ...
            cell_bbox = {'x': int(bbox_data['Left'] * width),
                'y': int(bbox_data['Top'] * height),
                'width': int(bbox_data['Width'] * width),
                'height': int(bbox_data['Height'] * height)}

            return {'text': cell_text.strip(), 'row_index': row_index,
                'column_index': column_index, 'bbox': cell_bbox,
    ...
    def get_service_info(self) -> Dict[str, Any]:
        return {'service_name': 'Amazon Textract',
            'region': self.config.get('ocr.aws_region', 'us-east-1'),
            'supports_tables': True, 'supports_forms': True, 'max_file_size': '10MB',
            'supported_formats': ['PNG', 'JPEG', 'PDF', 'TIFF'],
            'pricing_model': 'pay-per-use', 'rate_limits': True}
