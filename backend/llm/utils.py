    ...
    for i, question in enumerate(questions):
        ...
        if qid in answers:
            ...
            if answer_not_found and question_has_default:
                processed_answers[qid] = {'answer': deflection_defaults[question],
                    'page': 'Default',
                    'confidence': 95,
                ...
                processed_answers[qid] = {'answer': answer_text if answer_text else 'Not Found',
                    'page': page_info,
                    'confidence': confidence,
                    'source': 'OCR' if not answer_not_found else 'Not Found in Document'}
        else:
            processed_answers[qid] = {'answer': 'Not Found', 'page': None, 'confidence': 0, 'source': 'Missing'}
...
def normalize_answers_and_units(answers, questions):
    ...
    def parse_deflection_answer(raw_text, question_num):
        ...
        description = labels.get(question_num, "")

        results = []
        for m in re.finditer(r"[1L]\s*/\s*(\d{2,4})", text, flags=re.I):
            ratio = int(m.group(1))
    ...
    for i, question in enumerate(questions):
        ...
        if qnum == 1:
            ...
            result = parse_deflection_answer(raw_text, qnum)
            if result:
                defl_norm, defl_unit = result
...
def get_efficiency_metrics(results, combined_pages):
    ...
    return {'total_questions': len(results), 'successful_answers': len(successful_results),
        ...
        'total_pages_searched': total_pages_searched, 'avg_pages_per_question': avg_pages_per_question, 'total_pages_available': len(combined_pages),
...
def answers_with_coordinates(processed_answers: Dict[str, Dict], questions: List[str]) -> Dict[str, Dict]:
    ...
    for i, question in enumerate(questions, 1):
        ...
        coordinated_answer = {'answer': answer_data.get('answer', 'Not Found'),
            'page': answer_data.get('page', 'N/A'), 'confidence': answer_data.get('confidence', 0),
        ...
        if 'coordinates' in answer_data:
            ...
            if 'bounding_box' in coords:
                ...
                coordinated_answer['coordinate_summary'] = {'x': bbox.get('x', 0),
                    'y': bbox.get('y', 0),
...
def create_coordinate_json_output(results_data: List[Dict], output_path: str) -> None:
    output = {'metadata': {'coordinate_tracking_enabled': True,
            ...
            'coordinate_fields_explanation': {'coordinates': 'Full coordinate mapping data',
