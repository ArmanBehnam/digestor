"""
Advanced Prompt Engineering Module for LLM Optimization

This module provides comprehensive automatic prompt engineering capabilities
that analyze document context and questions to generate optimized prompts
for better answer extraction from engineering documents.
"""
...
class PromptEngineer:
    """
    Advanced prompt engineering system that dynamically optimizes prompts
    based on document context, question types, and engineering domains.
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Initialize OpenAI for prompt optimization (if available)
        ...
        # Prompt cache for optimization
        self.prompt_cache = {}
        
        # Engineering domain patterns
        self.domain_patterns = {
            'structural': ['beam', 'column', 'load', 'stress', 'deflection', 'steel', 'concrete'],
        ...
        self.question_types = {
            'identification': ['what is', 'what are', 'identify', 'name'],
    ...
    def optimize_prompt(self, page_text: str, questions: List[str]) -> str:
        """
        Generate an optimized prompt based on document context and questions.
        
        Args:
            page_text (str): The document text to analyze
            questions (List[str]): List of questions to answer
            
        Returns:
            str: Optimized prompt for better LLM performance
        """
        if not self.enabled:
        ...
        try:
            ...
            optimized_prompt = self._generate_optimized_prompt(
                page_text, questions, context_analysis, question_analysis
            )
    ...
    def _analyze_document_context(self, text: str) -> Dict[str, Any]:
        """Analyze document context to understand content type and structure."""
        analysis = {
            'domains': [],
            'key_terms': [],
        ...
        # Extract key technical terms (uppercase words, codes, standards)
        key_terms = re.findall(r'\\b[A-Z]{2,}\\b|\\b[A-Z]+\\d+\\b|\\b\\d+\\.\\d+\\b', text)
        analysis['key_terms'] = list(set(key_terms[:10]))  # Top 10 unique terms
    ...
    def _analyze_questions(self, questions: List[str]) -> Dict[str, Any]:
        """Analyze questions to understand what type of information is needed."""
        analysis = {
            'question_types': [],
            'domains_requested': [],
    ...
    def _generate_optimized_prompt(self, page_text: str, questions: List[str], 
                                 context_analysis: Dict, question_analysis: Dict) -> str:
        """Generate the optimized prompt based on all analysis."""
        ...
        domain_instructions = self._generate_domain_instructions(
            context_analysis['domains'], question_analysis['domains_requested']
        )
        ...
        # Generate error handling instructions
...
"""
    ...
    def _generate_system_message(self, context_analysis: Dict, question_analysis: Dict) -> str:
        """Generate an optimized system message."""
    ...
    def _generate_domain_instructions(self, doc_domains: List[str], 
                                    requested_domains: List[str]) -> str:
        """Generate domain-specific instructions."""
    ...
    def _generate_search_strategies(self, question_analysis: Dict) -> str:
        """Generate optimized search strategies based on question types."""
        ...
        # Add format-specific strategies
    ...
    def _generate_format_specifications(self, question_analysis: Dict) -> str:
        """Generate format specifications for answers."""
    ...
    def _generate_error_handling_instructions(self, context_analysis: Dict) -> str:
        """Generate instructions for handling OCR errors and ambiguities."""
    ...
    def _generate_static_prompt(self, page_text: str, questions: List[str]) -> str:
        """Generate the original static prompt when optimization is disabled."""
        qblock = "\\n".join([f"Q{i+1}. {q}" for i, q in enumerate(questions)])
    ...
    def _create_cache_key(self, text: str, questions: List[str]) -> str:
        """Create a cache key for prompt optimization."""
        text_hash = hash(text[:1000])  # Use first 1000 chars for efficiency
        questions_hash = hash(tuple(questions))
        return f"{text_hash}_{questions_hash}_{self.optimization_level}"
    
    def get_optimization_stats(self) -> Dict[str, Any]:
        """Get statistics about prompt optimization usage."""
        return {
